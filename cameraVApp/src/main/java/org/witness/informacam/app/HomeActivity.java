		...
		ListAdapterListener, OnMediaRecorderStopListener, ICacheWordSubscriber
{
	...
	private final static String LOG = Home.LOG;
...
//		if (USE_USER_MANAGEMENT_FRAGMENT == 1)
	...
	public void onResume()
	{
		...
		if (getIntent().hasExtra(Codes.Extras.GENERATING_KEY)) {
			mFirstTime = true;
			try {
                mainFragment.setIsGeneratingKey(getIntent().getBooleanExtra(Codes.Extras.GENERATING_KEY, false));
                getIntent().removeExtra(Codes.Extras.GENERATING_KEY);
